...
<head>
	...
	<title><?php echo $this->translate('Lost Password'); ?> | <?php echo esc_html(get_bloginfo('name')); ?></title>
	...
	<div class="<?php echo esc_attr($this->stand_alone_container_classes()); ?>">
		...
		<div class="content wrapper">
			<div class="content container <?php echo esc_attr($this->ui_widget_classes()); ?>">
				...
				<a name="lost_password" class="anchor"></a>

				<h1><?php echo $this->translate('Lost Password'); ?></h1>

				<?php echo $this->responses(); /* Errors, successes, and other messages. */ ?>

				<form method="post" action="<?php echo esc_attr($this->©url->current()); ?>#lost_password" class="lost-password ui-form">

					<?php $call = '©user_utils.®lost_password'; ?>
					...
					$form_fields->construct_field_markup(
						$form_fields->value($this->data->username_or_email),
						array(
						     'required'     => TRUE,
						     'autocomplete' => TRUE,
						     'type'         => 'text',
						     'name'         => $this->©action->input_name_for_call_arg(1),
						     'label'        => $this->translate('Username (or Email Address)')
						)
					);
					?>
