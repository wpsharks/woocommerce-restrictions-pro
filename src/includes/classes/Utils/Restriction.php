...
class Restriction extends SCoreClasses\SCore\Base\Core
{
    ...
    public function restrictsUriPatternsMetaBox(\WP_Post $post, array $args = [])
    {
        ...
        echo            '<li>'.__('It is possible to protect (and grant) access to portions of <em>/wp-admin/</em> with URI Patterns too! However, keep in mind that in order for a user to actually do anything in <em>/wp-admin/**</em> they will also need to have Capabilities which grant them permission. See: <strong>Role Capabilities</strong> as a form of protection if you\'d like more information.', 's2member-x').'</li>';
