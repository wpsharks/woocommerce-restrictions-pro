...
class ProductPermission extends SCoreClasses\SCore\Base\Core implements CoreInterfaces\SecondConstants
{
    ...
    public function __construct(Classes\App $App)
    {
        ...
        $this->access_offset_prefix     = __('after', 'woocommerce-restrictions');
        $this->access_offset_key_prefix = 'after'; // Hard-coded key.

        $this->expire_offset_suffix     = __('later', 'woocommerce-restrictions');
        ...
        $most_common_optgroup_key                                  = '--- '.__('Most Common', 'woocommerce-restrictions').' ---';
        $this->access_offset_directives[$most_common_optgroup_key] = '---'; // Creates an `<optgroup>`.

        $this->access_offset_directives['immediately'] = __('immediately', 'woocommerce-restrictions');

        # Define special expire offset directives.

        $this->expire_offset_directives[$most_common_optgroup_key] = '---'; // Creates an `<optgroup>`.

        $this->expire_offset_directives['naturally']          = __('naturally', 'woocommerce-restrictions');
        $this->expire_offset_directives['naturally -expired'] = __('naturally -expired', 'woocommerce-restrictions');
        $this->expire_offset_directives['never']              = __('never (unequivocally)', 'woocommerce-restrictions');

        # Period-based offset directives. Starting with minutes and working down from there.

        $minutes_optgroup_key                                  = '--- '.__('Minutes', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 60; $_i = $_i === 1 ? 5 : $_i + 5) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s minute', '%1$s %2$s minutes', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s minute %2$s', '%1$s minutes %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $hours_optgroup_key                                  = '--- '.__('Hours', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 24; $_i = $_i + 1) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s hour', '%1$s %2$s hours', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s hour %2$s', '%1$s hours %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $days_optgroup_key                                  = '--- '.__('Days', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 7; $_i = $_i + 1) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s day', '%1$s %2$s days', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s day %2$s', '%1$s days %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $upcoming_day_based_optgroup_key                                  = '--- '.__('Upcoming Weekday', 'woocommerce-restrictions').' ---';
        ...
        foreach ([ // Special cases made easy.
            'this sunday' => __('upcoming sunday', 'woocommerce-restrictions'),
            'this monday' => __('upcoming monday', 'woocommerce-restrictions'),
            'this tuesday' => __('upcoming tuesday', 'woocommerce-restrictions'),
            'this wednesday' => __('upcoming wednesday', 'woocommerce-restrictions'),
            'this thursday' => __('upcoming thursday', 'woocommerce-restrictions'),
            'this friday' => __('upcoming friday', 'woocommerce-restrictions'),
            'this saturday' => __('upcoming saturday', 'woocommerce-restrictions'),
        ...
        $next_day_based_optgroup_key                                  = '--- '.__('Next (After Upcoming) Day', 'woocommerce-restrictions').' ---';
        ...
        foreach ([ // Special cases made easy.
            'next sunday' => __('next sunday', 'woocommerce-restrictions'),
            'next monday' => __('next monday', 'woocommerce-restrictions'),
            'next tuesday' => __('next tuesday', 'woocommerce-restrictions'),
            'next wednesday' => __('next wednesday', 'woocommerce-restrictions'),
            'next thursday' => __('next thursday', 'woocommerce-restrictions'),
            'next friday' => __('next friday', 'woocommerce-restrictions'),
            'next saturday' => __('next saturday', 'woocommerce-restrictions'),
        ] as $_key => $_title) { // For access & expire.
            $this->access_offset_directives[$_key.' 12:00 am'] = $_title;
            $this->expire_offset_directives[$_key.' 11:59 pm'] = $_title;
        } // unset($_key, $_title); // Housekeeping.

        $weeks_optgroup_key                                  = '--- '.__('Weeks', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 4; $_i = $_i + 1) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s week', '%1$s %2$s weeks', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s week %2$s', '%1$s weeks %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $week_based_optgroup_key                                  = '--- '.__('Week-Based', 'woocommerce-restrictions').' ---';
        ...
        foreach ([ // Special cases made easy.
            'saturday this week' => __('last day (saturday) of current week', 'woocommerce-restrictions'),
            'monday next week' => __('first day (monday) of next week', 'woocommerce-restrictions'),
            'saturday next week' => __('last day (saturday) of next week', 'woocommerce-restrictions'),
        ] as $_key => $_title) { // For access & expire.
            $this->access_offset_directives[$_key.' 12:00 am'] = $_title;
            $this->expire_offset_directives[$_key.' 11:59 pm'] = $_title;
        } // unset($_key, $_title); // Housekeeping.

        $months_optgroup_key                                  = '--- '.__('Months', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 12; $_i = $_i + 1) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s month', '%1$s %2$s months', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s month %2$s', '%1$s months %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $month_based_optgroup_key                                  = '--- '.__('Month-Based', 'woocommerce-restrictions').' ---';
        ...
        foreach ([ // Special cases made easy.
            'last day of this month' => __('last day of current month', 'woocommerce-restrictions'),
            'first day of next month' => __('first day of next month', 'woocommerce-restrictions'),
            'last day of next month' => __('last day of next month', 'woocommerce-restrictions'),
        ] as $_key => $_title) { // For access & expire.
            $this->access_offset_directives[$_key.' 12:00 am'] = $_title;
            $this->expire_offset_directives[$_key.' 11:59 pm'] = $_title;
        } // unset($_key, $_title); // Housekeeping.

        $years_optgroup_key                                  = '--- '.__('Years', 'woocommerce-restrictions').' ---';
        ...
        for ($_i = 1; $_i <= 5; $_i = $_i + 1) {
            ...
            $this->access_offset_directives[$_access_key] = sprintf(_n('%1$s %2$s year', '%1$s %2$s years', $_i, 'woocommerce-restrictions'), $this->access_offset_prefix, $_i);
            $this->expire_offset_directives[$_expire_key] = sprintf(_n('%1$s year %2$s', '%1$s years %2$s', $_i, 'woocommerce-restrictions'), $_i, $this->expire_offset_suffix);
        } // unset($_i, $_access_key, $_expire_key); // Housekeeping.

        $year_based_optgroup_key                                  = '--- '.__('Year-Based', 'woocommerce-restrictions').' ---';
        ...
        foreach ([ // Special cases made easy.
            '1/1 next year -1 day' => __('last day of current year', 'woocommerce-restrictions'),
            '1/1 next year' => __('first day of next year', 'woocommerce-restrictions'),
        ] as $_key => $_title) { // For access & expire.
            $this->access_offset_directives[$_key.' 12:00 am'] = $_title;
            $this->expire_offset_directives[$_key.' 11:59 pm'] = $_title;
        } // unset($_key, $_title); // Housekeeping.
        ...
        if (s::applyFilters('enable_other_product_permission_offset_directives', !defined('WPLANG') || !WPLANG || mb_stripos(WPLANG, 'en') === 0)) {
            ...
            $custom_entry_optgroup_key                                  = '--- '.__('Custom Entry', 'woocommerce-restrictions').' ---';
            ...
            $this->access_offset_directives['other'] = $this->expire_offset_directives['other'] = __('other', 'woocommerce-restrictions');
        }
    }
